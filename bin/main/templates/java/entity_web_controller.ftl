...
public class ${EntityModel.displayName}WebController {
	...
	@GetMapping(value = "/web/${EntityModel.name}s")
	public String get${EntityModel.displayName}s(Model model, @RequestParam(value = "page", defaultValue = "1") int pageNumber) {
	    ...
	    return "${EntityModel.name}/${EntityModel.name}-list";
	}

	@GetMapping(value = { "/web/${EntityModel.name}s/add" })
	public String showAdd${EntityModel.displayName}(Model model) {
	    ...
	    return "${EntityModel.name}/${EntityModel.name}-add";
	}

	@PostMapping(value = "/web/${EntityModel.name}s/add")
	public String add${EntityModel.displayName}(@ModelAttribute ${EntityModel.displayName}RO t${EntityModel.displayName}RO,Model model)  {
		...
		model.addAttribute("${EntityModel.name}RO", t${EntityModel.displayName}RO);
		model.addAttribute("selectedMenu", "${EntityModel.name}");
		return "${EntityModel.name}/${EntityModel.name}-edit";
	}

	@GetMapping(value = { "/web/${EntityModel.name}s/{${EntityModel.name}Id}" })
	...
	@GetMapping(value = { "/web/${EntityModel.name}s/{${EntityModel.name}Id}/delete" })
	public String delete${EntityModel.displayName}ById(Model model, @PathVariable Long ${EntityModel.name}Id) throws RestException {
		...
		return "redirect:/web/${EntityModel.name}s";
	}
	
	<#list EntityModel.manyToManyRelations as relation>	 
	
	@GetMapping(value = { "/web/${EntityModel.name}s/{${EntityModel.name}Id}/${relation.name}s" })
	public String get${EntityModel.displayName}${relation.displayName}s(Model model, @PathVariable Long ${EntityModel.name}Id) throws RestException {
		...
		model.addAttribute("selectedMenu", "${EntityModel.name}");
		return "${EntityModel.name}/${relation.name}/${relation.name}-list";
	}
	
	@GetMapping(value = { "/web/${EntityModel.name}s/{${EntityModel.name}Id}/${relation.name}s/assign" })
	public String assign${relation.displayName}s(Model model, @PathVariable Long ${EntityModel.name}Id) throws RestException {
		...
		model.addAttribute("selectedMenu", "${EntityModel.name}");
		return "${EntityModel.name}/${relation.name}/${relation.name}-assign";
	}
	
	@GetMapping(value = { "/web/${EntityModel.name}s/{${EntityModel.name}Id}/${relation.name}s/{${relation.name}Id}/assign" })
	...
	@GetMapping(value = { "/web/${EntityModel.name}s/{${EntityModel.name}Id}/${relation.name}s/{${relation.name}Id}/unassign" })
